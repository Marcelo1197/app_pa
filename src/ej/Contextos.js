...
////////////////////////////////////////////////////////////
// S: hooks
function useProveedorDeContextoConsola() {
  //U: hook que provee state y opcional funciones
  const [unValor, setUnValor] = useState(""); //U: el estado que lo hace reactivo (propaga cambios)

  window.simularEventoExternoQueCambiaValor = function (valorNuevo) {
    //U: como una api externa, ej pa-api
  ...
  const cambiarValorDesdeReact = function (valorNuevo) {
    //U: llamando desde react
...
function useContextoConsola() {
  //U: con este hook los componentes que estan en el contexto pueden acceder
...
function ProvideContextoConsola({ children }) {
  //U: el componente para el contexto
  ...
  return (
    <div style={{ border: "1px solid black", margin: "10px", ...props.style }}>
      ...
      <p>
        ...
        <button onClick={() => contextoConsola.cambiarValorDesdeReact(miValor)}>
          Actualizar contexto
        </button>
      </p>
    </div>
...
function ComponenteIntermedioQueNoSeEnteraDeNadaUno() {
  return (
    <ComponenteQueUsaContexto nombre="Uno" style={{ background: "#FF8080" }} />
  );
}

function ComponenteIntermedioQueNoSeEnteraDeNadaDos() {
  return (
    <ComponenteQueUsaContexto nombre="Dos" style={{ background: "#80FF80" }} />
  );
}
...
export default function ComponenteQueLosEnvuelveYCreaElContexto() {
  return (
    <ProvideContextoConsola>
      ...
      <p>
        También podés usar en la consola{" "}
        <code>simularEventoExternoQueCambiaValor</code>
      </p>
